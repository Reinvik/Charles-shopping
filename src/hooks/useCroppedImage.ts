...
// LocalStorage key for cropped images
const CACHE_KEY = 'nexus_cropped_images_cache_v2';

// Load initial cache from localStorage
const loadCache = (): { [url: string]: string } => {
  try {
    const saved = localStorage.getItem(CACHE_KEY);
    return saved ? JSON.parse(saved) : {};
  } catch (e) {
    console.warn('Error loading crop cache from localStorage:', e);
    return {};
  }
};

const saveCache = (cache: { [url: string]: string }) => {
  try {
    // Prune cache if it grows too large to prevent QuotaExceededError
    const keys = Object.keys(cache);
    if (keys.length > 80) {
      // Keep only the last 50 items to keep size low
    ...
    console.warn('Error saving crop cache to localStorage:', e);
  }
};

const cropCache: { [url: string]: string } = loadCache();
...
export const useCroppedImage = (imageUrl: string) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      // Defer scanning to a timeout to avoid blocking page load and UI rendering
